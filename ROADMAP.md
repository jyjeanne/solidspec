...
| Schema-aware `/spcx:*` commands | `/spcx:<short>:{new,apply,finalise}` — reduced-schema-name namespaced, one per built-in schema (`/spcx:tdd:apply`, `/spcx:sec:new`, ...), no flagless default-schema shortcut — generated straight from each schema's own DAG (`src/agents/spcx.rs`), so the body always matches what that schema actually does. A fully custom-named schema gets `/spcx:<name>:*` too, unless its short name collides with a built-in's, in which case registration fails loudly instead of silently overwriting it; the not-yet-wired `unregister_commands` cleans up a custom schema's files symmetrically with whatever was registered for it. |
