...
## Knowledge graph (okf-rs)

A knowledge graph of the codebase lives in `docs/graph/`, generated by [okf-rs](https://github.com/jyjeanne/okf-rs) from local tree-sitter AST extraction (no LLM calls, nothing leaves the machine). Unlike a single opaque database, the output is an **OKF bundle**: one plain Markdown file with YAML frontmatter per concept (struct/function/module/...), cross-linked by ordinary Markdown links — it lives in the repo like any other file and its diffs are readable in a PR.

- `docs/graph/knowledge/` — the OKF bundle (queryable with `okf-rs search`/`explore`/`graph`/`diff`/`impact`)
- `docs/graph/GRAPH_REPORT.md` — summary: topology (`okf-rs graph stats`) and coverage (`okf-rs coverage`)
...
```bash
# One-time install of the CLI (not published to crates.io — install from git)
...
The incremental-index cache (`.okf-cache.json`, gitignored) speeds up repeat runs — only changed files are re-parsed.

### Querying (use this before broad greps)

When exploring unfamiliar parts of the codebase or assessing the blast radius of a change, query the bundle instead of scanning files:
