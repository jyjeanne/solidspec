...
A knowledge graph of the codebase lives in `docs/graph/`, generated by [graphify](https://github.com/Graphify-Labs/graphify) from local tree-sitter AST extraction (no LLM calls, nothing leaves the machine):

- `docs/graph/graph.json` — the queryable graph (nodes = modules/functions/structs, typed edges tagged `EXTRACTED`/`INFERRED`)
- `docs/graph/graph.html` — interactive visualization; open in a browser
- `docs/graph/GRAPH_REPORT.md` — summary: key concepts, hubs, communities
...
`.graphifyignore` excludes `docs/graph/` itself from extraction — keep it that way to avoid the graph indexing its own output.

### Querying (use this before broad greps)

When exploring unfamiliar parts of the codebase or assessing the blast radius of a change, query the graph instead of scanning files:

```bash
# Natural-language BFS traversal — where is X handled, what touches Y
graphify query "how are agent command files written" --graph docs/graph/graph.json

# Explain one symbol and its neighbors
graphify explain "ArtifactGraph" --graph docs/graph/graph.json

# Impact analysis: what is affected if X changes (use before refactors)
graphify affected "WorkflowSchema" --graph docs/graph/graph.json

# Architectural hubs — most-connected nodes
graphify god-nodes --graph docs/graph/graph.json --top 15

# Shortest dependency path between two symbols
graphify path "Cli" "ArtifactGraph" --graph docs/graph/graph.json
```

Use `graphify affected` on the symbols you are about to modify, and check `god-nodes` output when reviewing changes — edits to hub nodes deserve extra test coverage.
