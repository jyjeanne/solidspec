...
**Regression found by `/code-review` and fixed:** the paragraph above undersold the risk — for the `minimal` and `security-first` schemas specifically, the mismatch wasn't just "can't add new phases," it silently broke skip detection for *existing* phases. `filter_phases`'s `_` fallback arm routed both schemas to the generic 8-phase `PHASES` list (`specify, clarify, plan, tasks, tests, implement, analyze, review`), but neither schema's YAML declares `clarify`, `tests`, `analyze`, or `review` as artifacts. `should_skip`'s new schema-driven default arm does `graph.get(phase) → None → .unwrap_or(false)` for any phase absent from the schema graph — so those phases now **never skip**, re-running (and re-invoking the AI agent) on every pipeline invocation instead of stopping once already complete. Reproduced live: building the binary, writing `tests/story1.md` and `review-report.md`, then running `solidspec pipeline <feature> --dry-run --schema security-first --no-agent` showed `tests` and `review` both as `● run` instead of `○ skip`. Four independent review agents converged on this same bug from different angles (cross-file tracer, line-by-line diff scan with live reproduction, removed-behavior audit with executed-test verification).

**Fix applied:** added dedicated `PHASES_MINIMAL` (`specify, plan, tasks, implement`) and `PHASES_SECURITY_FIRST` (`specify, plan, security-review, tasks, implement`) phase-name arrays in `src/core/pipeline.rs`, matching each schema's actual artifact list, and wired them into `filter_phases`'s match so `"minimal"` and `"security-first"` no longer fall through to the generic `PHASES` list. `security-review` is a real artifact id in `security-first`'s graph, so it now correctly participates in schema-driven skip detection. Building a generic `security-review` phase executor for `execute_phase` (`cli/pipeline.rs`) remains out of scope — a live non-dry-run run of `security-first` now honestly bails with `Unknown phase: security-review` once it reaches that step, which is a deliberate improvement over the prior silent-wrong-phase-execution behavior. Regression tests added: `filter_minimal_phases_excludes_tests_and_review`, `filter_security_first_phases_includes_security_review`, `should_skip_security_review_when_report_exists`, `filter_phases_minimal_and_security_first_never_include_undeclared_phases` (`src/core/pipeline.rs`).
